...
if (!function_exists('dls_writing_desk_telegram_posts')) {
    function dls_writing_desk_telegram_posts() {
        $args = [
            'post_type'      => 'post',
            'post_status'    => ['draft', 'pending', 'future', 'publish', 'private'],
            'posts_per_page' => -1,
            ...
            'meta_query'     => [
                [
                    'key'     => '_dls_writing_desk_telegram_log',
                    'compare' => 'EXISTS',
                ],
            ],
        ];
        ...
        foreach ($posts as $post) {
            ...
            $log = dls_writing_desk_get_telegram_log($post->ID);
            if (empty($log)) {
                continue;
            }
            ...
            foreach ($log as $row) {
                ...
                if (in_array($status, ['sent', 'scheduled', 'failed'], true)) {
...
if (!function_exists('dls_writing_desk_normalize_telegram_username')) {
    function dls_writing_desk_normalize_telegram_username($value) {
        $value = trim((string) $value);
        if ($value === '') {
            return '';
        }
...
if (!function_exists('dls_writing_desk_telegram_username_from_meta')) {
    function dls_writing_desk_telegram_username_from_meta($type, $id) {
        $id = absint($id);
        if ($id < 1) {
            return '';
        }
        ...
        $keys = [
            '_dls_author_telegram',
            '_dls_guest_author_telegram',
            '_dls_author_telegram_username',
            '_dls_telegram_username',
            'telegram_username',
            'telegram',
        ];
        ...
        foreach ($keys as $key) {
            ...
            $username = dls_writing_desk_normalize_telegram_username($raw);
            if ($username !== '') {
                return $username;
            }
        }
...
if (!function_exists('dls_writing_desk_telegram_author_nicknames')) {
    function dls_writing_desk_telegram_author_nicknames($post_id) {
        $post_id = absint($post_id);
        if ($post_id < 1) {
            return [];
        }
        ...
        if (function_exists('dls_native_authors_get_post_people_with_roles')) {
            foreach ((array) dls_native_authors_get_post_people_with_roles($post_id) as $person) {
                if (($person['post_role'] ?? 'author') !== 'author') {
                    continue;
                }

                if (($person['author_type'] ?? 'user') === 'guest') {
                    $items[] = dls_writing_desk_telegram_username_from_meta('term', (int) ($person['term_id'] ?? 0));
                } else {
                    $items[] = dls_writing_desk_telegram_username_from_meta('user', (int) ($person['user_id'] ?? 0));
                }
            }
        } else {
            $selected = dls_writing_desk_parse_selection((string) (dls_writing_desk_get_selected_people($post_id)['author'] ?? ''));
            if (!empty($selected) && ($selected['author_type'] ?? 'user') === 'guest') {
                $items[] = dls_writing_desk_telegram_username_from_meta('term', (int) ($selected['term_id'] ?? 0));
            } elseif (!empty($selected)) {
                $items[] = dls_writing_desk_telegram_username_from_meta('user', (int) ($selected['user_id'] ?? 0));
            }
        }
...
if (!function_exists('dls_writing_desk_telegram_footer_links')) {
    function dls_writing_desk_telegram_footer_links() {
        $defaults = [
            ['label' => 'Канал', 'url' => 'https://t.me/DeadLawyers'],
            ['label' => 'Чят', 'url' => ''],
            ['label' => 'Вакансії', 'url' => 'https://jobs.deadlawyers.org'],
            ['label' => 'Події', 'url' => ''],
            ['label' => 'Кіноклуб', 'url' => ''],
        ];

        $stored = get_option('dls_writing_desk_telegram_footer_links', []);
        if (!is_array($stored)) {
            return $defaults;
        }
        ...
        foreach ($defaults as $index => $default) {
            $row = is_array($stored[$index] ?? null) ? $stored[$index] : [];
            $label = sanitize_text_field((string) ($row['label'] ?? $default['label']));
...
if (!function_exists('dls_writing_desk_telegram_footer_text')) {
    function dls_writing_desk_telegram_footer_text() {
        $parts = [];
        foreach (dls_writing_desk_telegram_footer_links() as $link) {
...
if (!function_exists('dls_writing_desk_telegram_text')) {
    function dls_writing_desk_telegram_text($post_id, $settings) {
        ...
        if (!empty($author_nicknames)) {
            $parts[] = implode(', ', array_map('esc_html', $author_nicknames));
        }

        $text = esc_html(implode("\n\n", array_filter($parts)));
        $footer = dls_writing_desk_telegram_footer_text();
...
if (!function_exists('dls_writing_desk_save_social_destinations')) {
    function dls_writing_desk_save_social_destinations() {
        ...
        $footer_rows = isset($_POST['dls_writing_desk_telegram_footer']) ? (array) wp_unslash($_POST['dls_writing_desk_telegram_footer']) : [];
        $footer_links = [];
        foreach (dls_writing_desk_telegram_footer_links() as $index => $default_link) {
            $row = is_array($footer_rows[$index] ?? null) ? $footer_rows[$index] : [];
            $label = sanitize_text_field((string) ($row['label'] ?? ($default_link['label'] ?? '')));
            $url = esc_url_raw((string) ($row['url'] ?? ($default_link['url'] ?? '')));
            $footer_links[] = [
                'label' => $label,
                'url'   => $url,
            ];
        }
        update_option('dls_writing_desk_telegram_footer_links', $footer_links, false);
...
if (!function_exists('dls_writing_desk_enqueue_assets')) {
    function dls_writing_desk_enqueue_assets($hook) {
        ...
        wp_add_inline_style('dls-writing-desk', '
            ...
            .dls-writing-desk__telegram-main {
                min-width: 0;
            }
            .dls-writing-desk__telegram-form {
                display: contents;
            }
            .dls-writing-desk__telegram-rail {
                position: sticky;
                top: 24px;
                display: flex;
            ...
            .dls-writing-desk__post-link--telegram {
                border-left: 4px solid #159bd6;
            }
            ...
            .dls-writing-desk__telegram-composer {
                display: flex;
                flex-direction: column;
                gap: 16px;
            }
            .dls-writing-desk__telegram-settings-card {
                border: 1px solid rgba(36,28,21,0.1);
                border-radius: 18px;
                background: rgba(255,255,255,0.82);
                overflow: hidden;
            }
            .dls-writing-desk__telegram-setting-row {
                display: flex;
                justify-content: space-between;
                gap: 14px;
                align-items: center;
                padding: 14px 16px;
                border-bottom: 1px solid rgba(36,28,21,0.08);
                font: 700 13px/1.35 "Helvetica Neue", Arial, sans-serif;
            }
            .dls-writing-desk__telegram-setting-row:last-child {
                border-bottom: 0;
            }
            .dls-writing-desk__telegram-setting-row input[type="checkbox"] {
                transform: scale(1.12);
            }
            .dls-writing-desk__telegram-channel-title {
                display: flex;
                gap: 10px;
                align-items: center;
                margin-bottom: 10px;
            }
            .dls-writing-desk__telegram-channel-dot {
                width: 34px;
                height: 34px;
                border-radius: 50%;
                display: inline-flex;
                ...
                background: #159bd6;
                color: #fff;
                font: 800 14px/1 "Helvetica Neue", Arial, sans-serif;
            }
        ...
        wp_add_inline_script('dls-writing-desk', '
            (function ($) {
                ...
                $(document).on("click", ".dls-writing-desk__remove-media", function (event) {
                    ...
                    if (preview) {
                        setPreview(preview, defaultImage, placeholder);
                    }
                    scheduleLocalDraftSave();
                });
...
if (!function_exists('dls_writing_desk_render_destinations_page')) {
    function dls_writing_desk_render_destinations_page() {
        ...
        $destinations = dls_writing_desk_get_social_destinations();
        $platforms = dls_writing_desk_destination_platforms();
        $telegram_footer_links = dls_writing_desk_telegram_footer_links();
        ...
        <div class="wrap dls-writing-desk">
            <div class="dls-writing-desk__admin-card">
                ...
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                    ...
                    <h2 style="margin-top:28px;">Telegram footer</h2>
                    <p class="dls-writing-desk__admin-sub" style="margin-bottom:12px;">These links are appended to every Telegram message.</p>
                    <table class="dls-writing-desk__admin-table widefat striped">
                        ...
                        <tbody>
                            <?php foreach ($telegram_footer_links as $index => $footer_link) : ?>
                                <tr>
                                    <td><input class="dls-writing-desk__input" type="text" name="dls_writing_desk_telegram_footer[<?php echo esc_attr((string) $index); ?>][label]" value="<?php echo esc_attr((string) ($footer_link['label'] ?? '')); ?>"></td>
                                    <td><input class="dls-writing-desk__input" type="url" name="dls_writing_desk_telegram_footer[<?php echo esc_attr((string) $index); ?>][url]" value="<?php echo esc_attr((string) ($footer_link['url'] ?? '')); ?>" placeholder="https://..."></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
...
if (!function_exists('dls_writing_desk_render_telegram_page')) {
    function dls_writing_desk_render_telegram_page() {
        ...
        $telegram_posts = dls_writing_desk_telegram_posts();
        $notice_message = dls_writing_desk_notice_message($_GET['desk_notice'] ?? '');
        ...
        <div class="wrap dls-writing-desk">
            <div class="dls-writing-desk__topbar">
                <div class="dls-writing-desk__brand">
                    ...
                    <h1 class="dls-writing-desk__title">Telegram Broadcast</h1>
                    <p class="dls-writing-desk__sub">Published Telegram posts on the left, message preview in the center, channel controls on the right.</p>
                </div>
                <div class="dls-writing-desk__actions">
            ...
            <div class="dls-writing-desk__shell dls-writing-desk__shell--telegram">
                <aside class="dls-writing-desk__panel">
                    <div class="dls-writing-desk__field">
                        <div class="dls-writing-desk__label"><span>Published Telegram</span><span><?php echo esc_html((string) count($telegram_posts)); ?></span></div>
                        <input id="dls-writing-desk-post-search" class="dls-writing-desk__input dls-writing-desk__input--search" type="search" data-target="#dls-writing-desk-post-list" placeholder="Search Telegram posts">
                    </div>
                    <div id="dls-writing-desk-post-list" class="dls-writing-desk__posts">
                        <?php if (empty($telegram_posts)) : ?>
                            <p class="dls-writing-desk__muted">No Telegram posts have been sent yet.</p>
                        <?php endif; ?>
                        <?php foreach ($telegram_posts as $recent_post) : ?>
                            <?php if (!($recent_post instanceof WP_Post)) { continue; } ?>
                            <?php
                            $is_active = $post instanceof WP_Post && $post->ID === $recent_post->ID;
                            $history_log = dls_writing_desk_get_telegram_log($recent_post->ID);
                            $history_bits = [];
                            ...
                            $history_label = !empty($history_bits) ? implode(', ', array_unique($history_bits)) : ucfirst((string) $recent_post->post_status);
                            ?>
                            <a class="dls-writing-desk__post-link dls-writing-desk__post-link--telegram<?php echo $is_active ? ' is-active' : ''; ?>" data-search="<?php echo esc_attr(strtolower(get_the_title($recent_post->ID) . ' ' . $history_label)); ?>" href="<?php echo esc_url(add_query_arg(['page' => 'dls-writing-desk-telegram', 'desk_post' => $recent_post->ID], admin_url('admin.php'))); ?>">
                                ...
                                <span class="dls-writing-desk__post-meta"><?php echo esc_html($history_label); ?> / <?php echo esc_html(get_the_modified_date('Y-m-d', $recent_post->ID)); ?></span>
                            </a>
                        <?php endforeach; ?>
                    </div>
                </aside>
                ...
                <?php if (!($post instanceof WP_Post)) : ?>
                    <section class="dls-writing-desk__editor dls-writing-desk__telegram-main">
                        <div class="dls-writing-desk__admin-card">
                            <h2 class="dls-writing-desk__admin-title">Choose a post</h2>
                            <p class="dls-writing-desk__admin-sub">Open a story from Writing Desk, then use Telegram Broadcast to prepare the channel messages.</p>
                            <div class="dls-writing-desk__panel-actions" style="margin-top:16px;">
                                <a class="dls-writing-desk__button dls-writing-desk__button--accent" href="<?php echo esc_url(admin_url('admin.php?page=dls-writing-desk')); ?>">Open Writing Desk</a>
                            </div>
                        </div>
                    </section>
                    <aside class="dls-writing-desk__telegram-rail">
                        <div class="dls-writing-desk__block">
                            <h3>Channels</h3>
                            <p class="dls-writing-desk__muted">Choose a post first.</p>
                        </div>
                    </aside>
                <?php else : ?>
                    <form class="dls-writing-desk__telegram-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                        ...
                        <section class="dls-writing-desk__editor dls-writing-desk__telegram-main">
                            <div class="dls-writing-desk__story-sheet dls-writing-desk__telegram-composer">
                                <div class="dls-writing-desk__block">
                            ...
                            <div class="dls-writing-desk__block">
                                ...
                                <?php else : ?>
                                    <?php foreach ($social_destinations as $destination) : ?>
                                        ...
                                        $featured_image_id = absint(get_post_thumbnail_id($post->ID));
                                        $featured_image_url = $featured_image_id > 0 ? (string) wp_get_attachment_image_url($featured_image_id, 'medium') : '';
                                        $social_image_url = $social_image_id > 0 ? (string) wp_get_attachment_image_url($social_image_id, 'medium') : $featured_image_url;
                                        $social_image_note = $social_image_id > 0 ? 'Custom Telegram image' : 'Featured image will be used by default';
                                        ...
                                        <div class="dls-writing-desk__social-card dls-writing-desk__social-card--telegram">
                                            <div class="dls-writing-desk__telegram-channel-title">
                                                <span class="dls-writing-desk__telegram-channel-dot">TG</span>
                                                <div>
                                                    <strong><?php echo esc_html((string) $destination['name']); ?></strong><br>
                                                    <span class="dls-writing-desk__muted"><?php echo esc_html((string) $destination['destination']); ?></span>
                                                </div>
                                            </div>
                                            ...
                                            <textarea class="dls-writing-desk__textarea" name="dls_writing_desk_social[<?php echo esc_attr($key); ?>][description]" placeholder="Message for this channel. If empty, the default text will be used."><?php echo esc_textarea((string) ($settings['description'] ?? '')); ?></textarea>

                                            <div class="dls-writing-desk__telegram-settings-card" style="margin-top:12px;">
                                                <label class="dls-writing-desk__telegram-setting-row"><span>Silent notification</span><input type="checkbox" name="dls_writing_desk_social[<?php echo esc_attr($key); ?>][silent]" value="1" <?php checked(!empty($settings['silent'])); ?>></label>
                                                <label class="dls-writing-desk__telegram-setting-row"><span>Pin</span><input type="checkbox" name="dls_writing_desk_social[<?php echo esc_attr($key); ?>][pin]" value="1" <?php checked(!empty($settings['pin'])); ?>></label>
                                                <label class="dls-writing-desk__telegram-setting-row"><span>Auto delete, min</span><input class="dls-writing-desk__input dls-writing-desk__input--mini" type="number" min="0" step="1" name="dls_writing_desk_social[<?php echo esc_attr($key); ?>][auto_delete]" value="<?php echo esc_attr((string) absint($settings['auto_delete'] ?? 0)); ?>"></label>
                                            </div>

                                            <div class="dls-writing-desk__field" style="margin-top:12px;">
                                                <div class="dls-writing-desk__label"><span>Buttons</span><span>Text or link</span></div>
                                            ...
                                            <div class="dls-writing-desk__thumb" style="margin-top:12px;">
                                                ...
                                                <p class="dls-writing-desk__muted" style="margin:0;"><?php echo esc_html($social_image_note); ?></p>
                                                <div class="dls-writing-desk__actions">
                                                    <button class="dls-writing-desk__button dls-writing-desk__button--soft dls-writing-desk__select-media" type="button" data-target="#<?php echo esc_attr($input_id); ?>" data-preview="#<?php echo esc_attr($preview_id); ?>" data-placeholder="Featured image will be used">Attach Photo</button>
                                                    <button class="dls-writing-desk__button dls-writing-desk__button--soft dls-writing-desk__remove-media" type="button" data-target="#<?php echo esc_attr($input_id); ?>" data-preview="#<?php echo esc_attr($preview_id); ?>" data-placeholder="Featured image will be used" data-default-image="<?php echo esc_attr($featured_image_url); ?>">Use Featured</button>
                                                </div>
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
