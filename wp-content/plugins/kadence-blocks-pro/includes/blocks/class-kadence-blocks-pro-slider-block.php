...
class Kadence_Blocks_Pro_Slider_Block extends Kadence_Blocks_Pro_Abstract_Block {
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		if ( ! empty( $attributes['kbVersion'] ) && $attributes['kbVersion'] > 1 ) {
			...
			$content            = sprintf( '<div %1$s><div class="kb-advanced-slider-inner-contain kb-adv-slider-html-version-2"><div class="kb-blocks-advanced-carousel kt-blocks-carousel kt-carousel-container-dotstyle-' . esc_attr( $dot_style ) . '"><div class="kb-blocks-advanced-slider-init kb-slider-loading kb-blocks-slider kt-carousel-arrowstyle-' . esc_attr( $arrow_style ) . ' kt-carousel-dotstyle-' . esc_attr( $dot_style ) . ' kb-slider-size-' . esc_attr( $height_type ) . ' kb-slider-type-' . esc_attr( $slider_type ) . ' kb-slider-ratio-' . esc_attr( $ratio ) . ' kb-slider-group-' . esc_attr( 'center' !== $arrow_position ? 'arrows' : 'arrow' ) . ' kb-slider-arrow-position-' . esc_attr( $arrow_position ) . ' kb-slider-tab-ratio-' . esc_attr( $tab_ratio ) . ' kb-slider-mobile-ratio-' . esc_attr( $mobile_ratio ) . ' splide kb-splide" data-slider-center-mode="' . esc_attr( $center_mode ) . '" data-slider-type="' . esc_attr( $slider_type ) . '" data-loop-type="' . esc_attr( $loop_type ) . '" data-columns-xxl="' . esc_attr( $columns[0] ) . '" data-columns-xl="' . esc_attr( $columns[1] ) . '" data-columns-md="' . esc_attr( $columns[2] ) . '" data-columns-sm="' . esc_attr( $columns[3] ) . '" data-columns-xs="' . esc_attr( $columns[4] ) . '" data-columns-ss="' . esc_attr( $columns[5] ) . '" data-slider-anim-speed="' . esc_attr( $anim_speed ) . '" data-slider-scroll="1" data-slider-fade="' . esc_attr( $fade ) . '" data-slider-arrows="' . esc_attr( 'none' === $arrow_style ? 'false' : 'true' ) . '" data-slider-dots="' . esc_attr( 'none' === $dot_style ? 'false' : 'true' ) . '" data-slider-hover-pause="' . esc_attr( $hover_pause ) . '" data-slider-auto="' . esc_attr( $auto_play ) . '"  data-dragging="' . esc_attr( $dragging ) . '" data-slider-speed="' . esc_attr( $auto_speed ) . '" data-slider-gap="' . esc_attr( $gap ) . '" data-slider-gap-tablet="' . esc_attr( $gap_tablet ) . '" data-slider-gap-mobile="' . esc_attr( $gap_mobile ) . '" data-slider-gap-unit="' . esc_attr( $gap_unit ) . '"><div class="splide__track"><div class="splide__list">%2$s</div></div></div></div></div></div>', $wrapper_attributes, $content );
		}
		return $content;
	}
