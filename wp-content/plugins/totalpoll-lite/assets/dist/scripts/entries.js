    ...
    var RepositoryService = /** @class */ (function () {
        ...
        RepositoryService.prototype.getDownload = function (format, filters) {
            ...
            var query = angular.extend({}, {
                action: this.prefix + "_entries_download",
                poll: this.pollId,
                format: format
            }, filters);
            var url = new URL(this.ajaxEndpoint);
            for (var queryParam in query) {
                if (query[queryParam]) {
                    url.searchParams.append(queryParam, query[queryParam]);
                }
            }
        ...
        RepositoryService.prototype.remove = function (id) {
            return this.resource.remove({
                action: this.prefix + "_entries_remove",
                id: id
            }).$promise;
        };
        RepositoryService.prototype.purge = function (pollId) {
            return this.resource.purge({
                action: this.prefix + "_entries_purge",
                poll: pollId
            }).$promise;
        };
...
(function (TotalPoll) {
    ...
    var EntriesBrowserComponent = /** @class */ (function (_super) {
        ...
        EntriesBrowserComponent.prototype.canPurge = function () {
            return this.entries.length > 0;
        };
         ...
         * @param $event
         */
        EntriesBrowserComponent.prototype.removeEntry = function (entry, $event) {
            var _this = this;
            ...
            if (confirmation) {
                ...
                this.RepositoryService.remove(entry.get('id')).then(function (response) {
                    if (response.success) {
        ...
        /**
         * Purge entries by poll.
         * @param entry
         * @param $event
         */
        EntriesBrowserComponent.prototype.purgeEntries = function ($event) {
            var _this = this;
            var confirmation = prompt('CAREFUL! This is an irreversible action. To proceed, please enter “purge” in the field below.', '');
            if (confirmation === 'purge') {
                this.startProcessing();
                // purge entries of a poll
                this.RepositoryService.purge(this.filters.poll).then(function (response) {
                    if (response.success) {
                        window.location.reload();
                    }
                    else {
                        alert('Unable to purge log');
                    }
                }).finally(function () {
                    _this.stopProcessing();
                });
